...
export class BinanceFuturesDataService {
    ...
    private async getCachedData(symbol: string, timeComponents: CacheKey): Promise<KlineData[] | null> {
        const result = await this.db.get(
            'SELECT data FROM futures_kline_cache WHERE symbol = ? AND hour = ? AND day = ? AND month = ? AND year = ?',
    ...
    private async cacheData(symbol: string, timeComponents: CacheKey, data: KlineData[]): Promise<void> {
        await this.db.run(
            'INSERT OR REPLACE INTO futures_kline_cache (symbol, hour, day, month, year, data, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ...
    public async getKlineData(symbol: string): Promise<KlineData[]> {
        ...
        if (cachedData) {
            console.log(`Returning cached futures data for ${symbol} at ${timeComponents.hour}:00 on ${timeComponents.day}/${timeComponents.month}/${timeComponents.year}`);
            return cachedData;
        }
        ...
        try {
            ...
            console.log(`Fetched and cached new futures data for ${symbol} at ${timeComponents.hour}:00 on ${timeComponents.day}/${timeComponents.month}/${timeComponents.year}`);
            return klineData;
        } catch (error) {
